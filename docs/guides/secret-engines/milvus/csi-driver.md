---
title: Mount Milvus Secrets using CSI Driver
menu:
  docs_{{ .version }}:
    identifier: csi-driver-milvus
    name: CSI Driver
...
# Mount Milvus Secrets using CSI Driver
...
> Note: YAML files used in this tutorial stored in [examples](/docs/examples/guides/secret-engines/milvus) folder in GitHub repository [KubeVault/docs](https://github.com/kubevault/kubevault)
...
## Enable & Configure Milvus SecretEngine

### Enable Milvus SecretEngine

```bash
$ kubectl apply -f docs/examples/guides/secret-engines/milvus/secretengine.yaml
...
```bash
$ kubectl apply -f docs/examples/guides/secret-engines/milvus/secretenginerole.yaml
milvusrole.engine.kubevault.com/milvus-superuser-role created
```
...
```bash
$ kubectl apply -f docs/examples/guides/secret-engines/milvus/serviceaccount.yaml
...
When a VaultPolicyBinding object is created, the KubeVault operator create an auth role in the Vault server. The role name is generated by the following naming format: `k8s.(clusterName or -).namespace.name`. Here, it is `k8s.kubevault.com.demo.milvus-superuser-role`.
...
```bash
$ kubectl apply -f docs/examples/guides/secret-engines/milvus/secretrolebinding.yaml
...
spec:
  ...
  parameters:
    ...
    roleName: "k8s.-.demo.milvus-reader-role"
    objects: |
      - objectName: "milvus-creds-username"
        secretPath: "your-database-path/creds/k8s.-.demo.milvus-superuser-role"
        secretKey: "username"
      - objectName: "milvus-creds-password"
        secretPath: "your-database-path/creds/k8s.-.demo.milvus-superuser-role"
        secretKey: "password"
...
```bash
$ kubectl apply -f docs/examples/guides/secret-engines/milvus/secretproviderclass.yaml
...
Now we can create a `Pod` to consume the `Milvus` secrets. When the `Pod` is created, the `Provider` fetches the secret and writes them to Pod's volume as files. At this point, the volume is successfully mounted and the `Pod` starts running.
    ...
    - image: jweissig/app:0.0.1
      ...
      volumeMounts:
        - name: secrets-store-inline
          mountPath: "/secrets-store/milvus-creds"
          readOnly: true
  volumes:
    - name: secrets-store-inline
      csi:
...
```bash
$ kubectl apply -f docs/examples/guides/secret-engines/milvus/pod.yaml
...
/ # ls /secrets-store/milvus-creds
milvus-creds-password  milvus-creds-username

/ # cat /secrets-store/milvus-creds/milvus-creds-password
TAu2Zvg1WYE07W8Uf-nW

/ # cat /secrets-store/milvus-creds/milvus-creds-username
v-kubernetes-test-k8s.-.demo.milvus-s-iPkxiH80Ollq2QgF82Ab-1629178048
