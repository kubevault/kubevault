---
title: Mount Qdrant Secrets using CSI Driver
menu:
  docs_{{ .version }}:
    identifier: csi-driver-qdrant
    name: CSI Driver
...
# Mount Qdrant Secrets using CSI Driver
...
> Note: The `qdrant-database-plugin` is **static-credentials-only** — it does not issue dynamic credentials. The CSI flow described below therefore reads from `database/static-creds/<role>`, which exposes a single API key (returned as the `password` field) that OpenBao rotates on the cadence configured by the operator via `bao write database/static-roles/<role>`. The `QdrantRole` CRD is a metadata binding only; the underlying Qdrant API key must already be wired into the Qdrant server's `QDRANT__SERVICE__API_KEY` environment variable.
...
> Note: YAML files used in this tutorial stored in [examples](/docs/examples/guides/secret-engines/qdrant) folder in GitHub repository [KubeVault/docs](https://github.com/kubevault/kubevault)
...
## Enable & Configure Qdrant SecretEngine

### Enable Qdrant SecretEngine

```bash
$ kubectl apply -f docs/examples/guides/secret-engines/qdrant/secretengine.yaml
...
```bash
$ kubectl apply -f docs/examples/guides/secret-engines/qdrant/secretenginerole.yaml
qdrantrole.engine.kubevault.com/qdrant-app-role created
```
...
```bash
$ kubectl apply -f docs/examples/guides/secret-engines/qdrant/serviceaccount.yaml
...
When a VaultPolicyBinding object is created, the KubeVault operator creates an auth role in the Vault server. The role name is generated by the following naming format: `k8s.(clusterName or -).namespace.name`. Here, it is `k8s.-.demo.qdrant-app-role`.
...
```bash
$ kubectl apply -f docs/examples/guides/secret-engines/qdrant/secretrolebinding.yaml
...
Create `SecretProviderClass` object with the following content. Because the plugin is static-only and Qdrant authenticates with a single API key, the `secretPath` points at `database/static-creds/<role>` and we only mount one object (`password`):
...
spec:
  ...
  parameters:
    ...
    roleName: "k8s.-.demo.qdrant-app-role"
    objects: |
      - objectName: "qdrant-creds-api-key"
        secretPath: "your-database-path/static-creds/k8s.-.demo.qdrant-app-role"
        secretKey: "password"
```

```bash
$ kubectl apply -f docs/examples/guides/secret-engines/qdrant/secretproviderclass.yaml
...
Now we can create a `Pod` to consume the `Qdrant` API key. When the `Pod` is created, the `Provider` fetches the secret and writes it to Pod's volume as a file. At this point, the volume is successfully mounted and the `Pod` starts running.
    ...
    - image: jweissig/app:0.0.1
      ...
      volumeMounts:
        - name: secrets-store-inline
          mountPath: "/secrets-store/qdrant-creds"
          readOnly: true
  volumes:
    - name: secrets-store-inline
      csi:
...
```bash
$ kubectl apply -f docs/examples/guides/secret-engines/qdrant/pod.yaml
...
/ # ls /secrets-store/qdrant-creds
qdrant-creds-api-key

/ # cat /secrets-store/qdrant-creds/qdrant-creds-api-key
TAu2Zvg1WYE07W8Uf-nW

/ # exit
```

So, we can see that the rotated Qdrant API key is mounted into the pod as a single file. As OpenBao rotates the key on the configured cadence, the CSI driver will refresh the mounted file in place.
