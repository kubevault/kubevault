...
The `QdrantRole` specifies the collection-level access permissions encoded into the dynamic JWT:
...
Now we can create a `Pod` to consume the dynamic `Qdrant` JWT token. When the `Pod` is created, the `Provider` fetches the secret and writes it to Pod's volume as a file. At this point, the volume is successfully mounted and the `Pod` starts running.
...
/ # cat /secrets-store/qdrant-creds/qdrant-jwt-token
eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOi...

/ # exit
```

The dynamic JWT token is mounted into the pod. The application can read this file and present it to Qdrant via the `api-key` header or `Authorization: Bearer <token>`.
