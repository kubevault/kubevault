---
title: Mount Apache ZooKeeper Secrets using CSI Driver
menu:
  docs_{{ .version }}:
    identifier: csi-driver-zookeeper
    name: CSI Driver
...
# Mount Apache ZooKeeper Secrets using CSI Driver
...
> Note: The `zookeeper-database-plugin` is **static-credentials-only** — it does not issue dynamic credentials. The CSI flow described below therefore reads from `database/static-creds/<role>`, which exposes a SASL username/password pair that OpenBao rotates on the cadence configured by the operator via `bao write database/static-roles/<role>`. The `ZooKeeperRole` CRD is a metadata binding only; the underlying ZooKeeper principal must already exist in the ensemble's JAAS configuration.
...
> Note: YAML files used in this tutorial stored in [examples](/docs/examples/guides/secret-engines/zookeeper) folder in GitHub repository [KubeVault/docs](https://github.com/kubevault/kubevault)
...
## Enable & Configure Apache ZooKeeper SecretEngine

### Enable Apache ZooKeeper SecretEngine

```bash
$ kubectl apply -f docs/examples/guides/secret-engines/zookeeper/secretengine.yaml
secretengine.engine.kubevault.com/zookeeper-engine created
```

### Create ZooKeeperRole

```bash
$ kubectl apply -f docs/examples/guides/secret-engines/zookeeper/secretenginerole.yaml
zookeeperrole.engine.kubevault.com/zookeeper-app-role created
```
...
```bash
$ kubectl apply -f docs/examples/guides/secret-engines/zookeeper/serviceaccount.yaml
...
When a VaultPolicyBinding object is created, the KubeVault operator creates an auth role in the Vault server. The role name is generated by the following naming format: `k8s.(clusterName or -).namespace.name`. Here, it is `k8s.-.demo.zookeeper-app-role`.
...
```bash
$ kubectl apply -f docs/examples/guides/secret-engines/zookeeper/secretrolebinding.yaml
...
spec:
  ...
  parameters:
    ...
    roleName: "k8s.-.demo.zookeeper-app-role"
    objects: |
      - objectName: "zookeeper-creds-username"
        secretPath: "your-database-path/static-creds/k8s.-.demo.zookeeper-app-role"
        secretKey: "username"
      - objectName: "zookeeper-creds-password"
        secretPath: "your-database-path/static-creds/k8s.-.demo.zookeeper-app-role"
        secretKey: "password"
```

```bash
$ kubectl apply -f docs/examples/guides/secret-engines/zookeeper/secretproviderclass.yaml
...
Now we can create a `Pod` to consume the `Apache ZooKeeper` secrets. When the `Pod` is created, the `Provider` fetches the secret and writes them to Pod's volume as files. At this point, the volume is successfully mounted and the `Pod` starts running.
    ...
    - image: jweissig/app:0.0.1
      ...
      volumeMounts:
        - name: secrets-store-inline
          mountPath: "/secrets-store/zookeeper-creds"
          readOnly: true
  volumes:
    - name: secrets-store-inline
      csi:
...
```bash
$ kubectl apply -f docs/examples/guides/secret-engines/zookeeper/pod.yaml
...
/ # ls /secrets-store/zookeeper-creds
zookeeper-creds-password  zookeeper-creds-username

/ # cat /secrets-store/zookeeper-creds/zookeeper-creds-username
zk-app-user

/ # cat /secrets-store/zookeeper-creds/zookeeper-creds-password
TAu2Zvg1WYE07W8Uf-nW

/ # exit
```

So, we can see that the rotated static credentials for the ZooKeeper principal are mounted into the pod, where each secret key is mounted as a file and the value is the content of that file. As OpenBao rotates the principal on the configured cadence, the CSI driver will refresh the mounted files in place.
