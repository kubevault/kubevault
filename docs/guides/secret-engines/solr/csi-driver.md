---
title: Mount Apache Solr Secrets using CSI Driver
menu:
  docs_{{ .version }}:
    identifier: csi-driver-solr
    name: CSI Driver
...
# Mount Apache Solr Secrets using CSI Driver
...
> Note: YAML files used in this tutorial stored in [examples](/docs/examples/guides/secret-engines/solr) folder in GitHub repository [KubeVault/docs](https://github.com/kubevault/kubevault)
...
## Enable & Configure Apache Solr SecretEngine

### Enable Apache Solr SecretEngine

```bash
$ kubectl apply -f docs/examples/guides/secret-engines/solr/secretengine.yaml
...
```bash
$ kubectl apply -f docs/examples/guides/secret-engines/solr/secretenginerole.yaml
solrrole.engine.kubevault.com/solr-superuser-role created
```
...
```bash
$ kubectl apply -f docs/examples/guides/secret-engines/solr/serviceaccount.yaml
...
When a VaultPolicyBinding object is created, the KubeVault operator create an auth role in the Vault server. The role name is generated by the following naming format: `k8s.(clusterName or -).namespace.name`. Here, it is `k8s.kubevault.com.demo.solr-superuser-role`.
...
```bash
$ kubectl apply -f docs/examples/guides/secret-engines/solr/secretrolebinding.yaml
...
spec:
  ...
  parameters:
    ...
    roleName: "k8s.-.demo.solr-reader-role"
    objects: |
      - objectName: "solr-creds-username"
        secretPath: "your-database-path/creds/k8s.-.demo.solr-superuser-role"
        secretKey: "username"
      - objectName: "solr-creds-password"
        secretPath: "your-database-path/creds/k8s.-.demo.solr-superuser-role"
        secretKey: "password"
...
```bash
$ kubectl apply -f docs/examples/guides/secret-engines/solr/secretproviderclass.yaml
...
Now we can create a `Pod` to consume the `Solr` secrets. When the `Pod` is created, the `Provider` fetches the secret and writes them to Pod's volume as files. At this point, the volume is successfully mounted and the `Pod` starts running.
    ...
    - image: jweissig/app:0.0.1
      ...
      volumeMounts:
        - name: secrets-store-inline
          mountPath: "/secrets-store/solr-creds"
          readOnly: true
  volumes:
    - name: secrets-store-inline
      csi:
...
```bash
$ kubectl apply -f docs/examples/guides/secret-engines/solr/pod.yaml
...
/ # ls /secrets-store/solr-creds
solr-creds-password  solr-creds-username

/ # cat /secrets-store/solr-creds/solr-creds-password
TAu2Zvg1WYE07W8Uf-nW

/ # cat /secrets-store/solr-creds/solr-creds-username
v-kubernetes-test-k8s.-.demo.solr-s-iPkxiH80Ollq2QgF82Ab-1629178048
