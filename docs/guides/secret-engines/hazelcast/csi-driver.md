---
title: Mount Hazelcast Secrets using CSI Driver
menu:
  docs_{{ .version }}:
    identifier: csi-driver-hazelcast
    name: CSI Driver
...
# Mount Hazelcast Secrets using CSI Driver
...
> Note: The `hazelcast-database-plugin` is **static-credentials-only** — it does not issue dynamic credentials. The CSI flow described below therefore reads from `database/static-creds/<role>`, which exposes a username/password pair that OpenBao rotates on the cadence configured by the operator via `bao write database/static-roles/<role>`. The `HazelcastRole` CRD is a metadata binding only; the underlying Hazelcast principal must already exist in the cluster's authentication realm.
...
> Note: YAML files used in this tutorial stored in [examples](/docs/examples/guides/secret-engines/hazelcast) folder in GitHub repository [KubeVault/docs](https://github.com/kubevault/kubevault)
...
## Enable & Configure Hazelcast SecretEngine

### Enable Hazelcast SecretEngine

```bash
$ kubectl apply -f docs/examples/guides/secret-engines/hazelcast/secretengine.yaml
secretengine.engine.kubevault.com/hazelcast-engine created
```

### Create HazelcastRole

```bash
$ kubectl apply -f docs/examples/guides/secret-engines/hazelcast/secretenginerole.yaml
hazelcastrole.engine.kubevault.com/hazelcast-app-role created
```
...
```bash
$ kubectl apply -f docs/examples/guides/secret-engines/hazelcast/serviceaccount.yaml
...
When a VaultPolicyBinding object is created, the KubeVault operator creates an auth role in the Vault server. The role name is generated by the following naming format: `k8s.(clusterName or -).namespace.name`. Here, it is `k8s.-.demo.hazelcast-app-role`.
...
```bash
$ kubectl apply -f docs/examples/guides/secret-engines/hazelcast/secretrolebinding.yaml
...
spec:
  ...
  parameters:
    ...
    roleName: "k8s.-.demo.hazelcast-app-role"
    objects: |
      - objectName: "hazelcast-creds-username"
        secretPath: "your-database-path/static-creds/k8s.-.demo.hazelcast-app-role"
        secretKey: "username"
      - objectName: "hazelcast-creds-password"
        secretPath: "your-database-path/static-creds/k8s.-.demo.hazelcast-app-role"
        secretKey: "password"
```

```bash
$ kubectl apply -f docs/examples/guides/secret-engines/hazelcast/secretproviderclass.yaml
...
Now we can create a `Pod` to consume the `Hazelcast` secrets. When the `Pod` is created, the `Provider` fetches the secret and writes them to Pod's volume as files. At this point, the volume is successfully mounted and the `Pod` starts running.
    ...
    - image: jweissig/app:0.0.1
      ...
      volumeMounts:
        - name: secrets-store-inline
          mountPath: "/secrets-store/hazelcast-creds"
          readOnly: true
  volumes:
    - name: secrets-store-inline
      csi:
...
```bash
$ kubectl apply -f docs/examples/guides/secret-engines/hazelcast/pod.yaml
...
/ # ls /secrets-store/hazelcast-creds
hazelcast-creds-password  hazelcast-creds-username

/ # cat /secrets-store/hazelcast-creds/hazelcast-creds-username
hz-app-user

/ # cat /secrets-store/hazelcast-creds/hazelcast-creds-password
TAu2Zvg1WYE07W8Uf-nW

/ # exit
```

So, we can see that the rotated static credentials for the Hazelcast principal are mounted into the pod, where each secret key is mounted as a file and the value is the content of that file. As OpenBao rotates the principal on the configured cadence, the CSI driver will refresh the mounted files in place.
