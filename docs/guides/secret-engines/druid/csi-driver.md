---
title: Mount Apache Druid Secrets using CSI Driver
menu:
  docs_{{ .version }}:
    identifier: csi-driver-druid
    name: CSI Driver
...
# Mount Apache Druid Secrets using CSI Driver
...
> Note: YAML files used in this tutorial stored in [examples](/docs/examples/guides/secret-engines/druid) folder in GitHub repository [KubeVault/docs](https://github.com/kubevault/kubevault)
...
## Enable & Configure Apache Druid SecretEngine

### Enable Apache Druid SecretEngine

```bash
$ kubectl apply -f docs/examples/guides/secret-engines/druid/secretengine.yaml
...
```bash
$ kubectl apply -f docs/examples/guides/secret-engines/druid/secretenginerole.yaml
druidrole.engine.kubevault.com/druid-superuser-role created
```
...
```bash
$ kubectl apply -f docs/examples/guides/secret-engines/druid/serviceaccount.yaml
...
When a VaultPolicyBinding object is created, the KubeVault operator creates an auth role in the Vault server. The role name is generated by the following naming format: `k8s.(clusterName or -).namespace.name`. Here, it is `k8s.-.demo.druid-reader-role`.
...
```bash
$ kubectl apply -f docs/examples/guides/secret-engines/druid/secretrolebinding.yaml
...
spec:
  ...
  parameters:
    ...
    roleName: "k8s.-.demo.druid-reader-role"
    objects: |
      - objectName: "druid-creds-username"
        secretPath: "your-database-path/creds/k8s.-.demo.druid-superuser-role"
        secretKey: "username"
      - objectName: "druid-creds-password"
        secretPath: "your-database-path/creds/k8s.-.demo.druid-superuser-role"
        secretKey: "password"
...
```bash
$ kubectl apply -f docs/examples/guides/secret-engines/druid/secretproviderclass.yaml
...
Now we can create a `Pod` to consume the `Druid` secrets. When the `Pod` is created, the `Provider` fetches the secret and writes them to Pod's volume as files. At this point, the volume is successfully mounted and the `Pod` starts running.
    ...
    - image: jweissig/app:0.0.1
      ...
      volumeMounts:
        - name: secrets-store-inline
          mountPath: "/secrets-store/druid-creds"
          readOnly: true
  volumes:
    - name: secrets-store-inline
      csi:
...
```bash
$ kubectl apply -f docs/examples/guides/secret-engines/druid/pod.yaml
...
/ # ls /secrets-store/druid-creds
druid-creds-password  druid-creds-username

/ # cat /secrets-store/druid-creds/druid-creds-password
TAu2Zvg1WYE07W8Uf-nW

/ # cat /secrets-store/druid-creds/druid-creds-username
v-kubernetes-test-k8s---demo-druid-s-iPkxiH80Ollq2QgF82Ab-1629178048

/ # exit
```

So, we can see that the secret `druid-creds-username` and `druid-creds-password` is mounted into the pod, where the secret key is mounted as file and value is the content of that file.
