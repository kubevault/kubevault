...
Now we can create a `Pod` to consume the `AWS` secrets. When the `Pod` is created, the `Provider` fetches the secret and write them to `Pod` volume as files. At this point, the volume is successfully mounted and the `Pod` starts running.
