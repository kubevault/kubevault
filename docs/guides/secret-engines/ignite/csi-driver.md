---
title: Mount Apache Ignite Secrets using CSI Driver
menu:
  docs_{{ .version }}:
    identifier: csi-driver-ignite
    name: CSI Driver
...
# Mount Apache Ignite Secrets using CSI Driver
...
> Note: YAML files used in this tutorial stored in [examples](/docs/examples/guides/secret-engines/ignite) folder in GitHub repository [KubeVault/docs](https://github.com/kubevault/kubevault)
...
## Enable & Configure Apache Ignite SecretEngine

### Enable Apache Ignite SecretEngine

```bash
$ kubectl apply -f docs/examples/guides/secret-engines/ignite/secretengine.yaml
...
```bash
$ kubectl apply -f docs/examples/guides/secret-engines/ignite/secretenginerole.yaml
igniterole.engine.kubevault.com/ignite-superuser-role created
```
...
```bash
$ kubectl apply -f docs/examples/guides/secret-engines/ignite/serviceaccount.yaml
...
When a VaultPolicyBinding object is created, the KubeVault operator creates an auth role in the Vault server. The role name is generated by the following naming format: `k8s.(clusterName or -).namespace.name`. Here, it is `k8s.-.demo.ignite-reader-role`.
...
```bash
$ kubectl apply -f docs/examples/guides/secret-engines/ignite/secretrolebinding.yaml
...
spec:
  ...
  parameters:
    ...
    roleName: "k8s.-.demo.ignite-reader-role"
    objects: |
      - objectName: "ignite-creds-username"
        secretPath: "your-database-path/creds/k8s.-.demo.ignite-superuser-role"
        secretKey: "username"
      - objectName: "ignite-creds-password"
        secretPath: "your-database-path/creds/k8s.-.demo.ignite-superuser-role"
        secretKey: "password"
...
```bash
$ kubectl apply -f docs/examples/guides/secret-engines/ignite/secretproviderclass.yaml
...
Now we can create a `Pod` to consume the `Apache Ignite` secrets. When the `Pod` is created, the `Provider` fetches the secret and writes them to Pod's volume as files. At this point, the volume is successfully mounted and the `Pod` starts running.
    ...
    - image: jweissig/app:0.0.1
      ...
      volumeMounts:
        - name: secrets-store-inline
          mountPath: "/secrets-store/ignite-creds"
          readOnly: true
  volumes:
    - name: secrets-store-inline
      csi:
...
```bash
$ kubectl apply -f docs/examples/guides/secret-engines/ignite/pod.yaml
...
/ # ls /secrets-store/ignite-creds
ignite-creds-password  ignite-creds-username

/ # cat /secrets-store/ignite-creds/ignite-creds-password
TAu2Zvg1WYE07W8Uf-nW

/ # cat /secrets-store/ignite-creds/ignite-creds-username
V_KUBERNETES_TEST_K8S___DEMO_IGNITE_S_IPKXIH80OLLQ2QGF82AB_1629178048

/ # exit
```

So, we can see that the secret `ignite-creds-username` and `ignite-creds-password` is mounted into the pod, where the secret key is mounted as file and value is the content of that file.
