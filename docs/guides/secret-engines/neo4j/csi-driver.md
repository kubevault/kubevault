---
title: Mount Neo4j Secrets using CSI Driver
menu:
  docs_{{ .version }}:
    identifier: csi-driver-neo4j
    name: CSI Driver
...
# Mount Neo4j Secrets using CSI Driver
...
> Note: YAML files used in this tutorial stored in [examples](/docs/examples/guides/secret-engines/neo4j) folder in GitHub repository [KubeVault/docs](https://github.com/kubevault/kubevault)
...
## Enable & Configure Neo4j SecretEngine

### Enable Neo4j SecretEngine

```bash
$ kubectl apply -f docs/examples/guides/secret-engines/neo4j/secretengine.yaml
...
```bash
$ kubectl apply -f docs/examples/guides/secret-engines/neo4j/secretenginerole.yaml
neo4jrole.engine.kubevault.com/neo4j-superuser-role created
```
...
```bash
$ kubectl apply -f docs/examples/guides/secret-engines/neo4j/serviceaccount.yaml
...
When a VaultPolicyBinding object is created, the KubeVault operator create an auth role in the Vault server. The role name is generated by the following naming format: `k8s.(clusterName or -).namespace.name`. Here, it is `k8s.kubevault.com.demo.neo4j-superuser-role`.
...
```bash
$ kubectl apply -f docs/examples/guides/secret-engines/neo4j/secretrolebinding.yaml
...
spec:
  ...
  parameters:
    ...
    roleName: "k8s.-.demo.neo4j-reader-role"
    objects: |
      - objectName: "neo4j-creds-username"
        secretPath: "your-database-path/creds/k8s.-.demo.neo4j-superuser-role"
        secretKey: "username"
      - objectName: "neo4j-creds-password"
        secretPath: "your-database-path/creds/k8s.-.demo.neo4j-superuser-role"
        secretKey: "password"
...
```bash
$ kubectl apply -f docs/examples/guides/secret-engines/neo4j/secretproviderclass.yaml
...
Now we can create a `Pod` to consume the `Neo4j` secrets. When the `Pod` is created, the `Provider` fetches the secret and writes them to Pod's volume as files. At this point, the volume is successfully mounted and the `Pod` starts running.
    ...
    - image: jweissig/app:0.0.1
      ...
      volumeMounts:
        - name: secrets-store-inline
          mountPath: "/secrets-store/neo4j-creds"
          readOnly: true
  volumes:
    - name: secrets-store-inline
      csi:
...
```bash
$ kubectl apply -f docs/examples/guides/secret-engines/neo4j/pod.yaml
...
/ # ls /secrets-store/neo4j-creds
neo4j-creds-password  neo4j-creds-username

/ # cat /secrets-store/neo4j-creds/neo4j-creds-password
TAu2Zvg1WYE07W8Uf-nW

/ # cat /secrets-store/neo4j-creds/neo4j-creds-username
v-kubernetes-test-k8s.-.demo.neo4j-s-iPkxiH80Ollq2QgF82Ab-1629178048
