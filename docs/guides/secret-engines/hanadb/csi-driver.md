---
title: Mount SAP HANA Secrets using CSI Driver
menu:
  docs_{{ .version }}:
    identifier: csi-driver-hanadb
    name: CSI Driver
...
# Mount SAP HANA Secrets using CSI Driver
...
> Note: YAML files used in this tutorial stored in [examples](/docs/examples/guides/secret-engines/hanadb) folder in GitHub repository [KubeVault/docs](https://github.com/kubevault/kubevault)
...
## Enable & Configure SAP HANA SecretEngine

### Enable SAP HANA SecretEngine

```bash
$ kubectl apply -f docs/examples/guides/secret-engines/hanadb/secretengine.yaml
...
```bash
$ kubectl apply -f docs/examples/guides/secret-engines/hanadb/secretenginerole.yaml
...
```bash
$ kubectl apply -f docs/examples/guides/secret-engines/hanadb/serviceaccount.yaml
...
When a VaultPolicyBinding object is created, the KubeVault operator creates an auth role in the Vault server. The role name is generated by the following naming format: `k8s.(clusterName or -).namespace.name`. Here, it is `k8s.-.demo.hanadb-reader-role`.
...
```bash
$ kubectl apply -f docs/examples/guides/secret-engines/hanadb/secretrolebinding.yaml
...
spec:
  ...
  parameters:
    ...
    roleName: "k8s.-.demo.hanadb-reader-role"
    objects: |
      - objectName: "hanadb-creds-username"
        secretPath: "your-database-path/creds/k8s.-.demo.hanadb-readonly"
        secretKey: "username"
      - objectName: "hanadb-creds-password"
        secretPath: "your-database-path/creds/k8s.-.demo.hanadb-readonly"
        secretKey: "password"
...
```bash
$ kubectl apply -f docs/examples/guides/secret-engines/hanadb/secretproviderclass.yaml
...
Now we can create a `Pod` to consume the `SAP HANA` secrets. When the `Pod` is created, the `Provider` fetches the secret and writes them to Pod's volume as files. At this point, the volume is successfully mounted and the `Pod` starts running.
    ...
    - image: jweissig/app:0.0.1
      ...
      volumeMounts:
        - name: secrets-store-inline
          mountPath: "/secrets-store/hanadb-creds"
          readOnly: true
  volumes:
    - name: secrets-store-inline
      csi:
...
```bash
$ kubectl apply -f docs/examples/guides/secret-engines/hanadb/pod.yaml
...
/ # ls /secrets-store/hanadb-creds
hanadb-creds-password  hanadb-creds-username

/ # cat /secrets-store/hanadb-creds/hanadb-creds-password
TAu2Zvg1WYE07W8Uf-nW

/ # cat /secrets-store/hanadb-creds/hanadb-creds-username
V_KUBERNETES_TEST_K8S___DEMO_HANADB_S_IPKXIH80OLLQ2QGF82AB_1629178048

/ # exit
```

So, we can see that the secret `hanadb-creds-username` and `hanadb-creds-password` is mounted into the pod, where the secret key is mounted as file and value is the content of that file.
