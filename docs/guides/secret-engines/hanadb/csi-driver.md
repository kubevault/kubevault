...
hanadbrole.engine.kubevault.com/hanadb-superuser-role created
```
...
When a VaultPolicyBinding object is created, the KubeVault operator create an auth role in the Vault server. The role name is generated by the following naming format: `k8s.(clusterName or -).namespace.name`. Here, it is `k8s.kubevault.com.demo.hanadb-superuser-role`.
  ...
  parameters:
    ...
    objects: |
      - objectName: "hanadb-creds-username"
        secretPath: "your-database-path/creds/k8s.-.demo.hanadb-superuser-role"
        secretKey: "username"
...
v-kubernetes-test-k8s.-.demo.hanadb-s-iPkxiH80Ollq2QgF82Ab-1629178048
