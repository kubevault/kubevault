...
The `WeaviateRole` specifies the roles assigned to the dynamically created Weaviate user:
...
Create `SecretProviderClass` object with the following content. The `secretPath` points at `database/creds/<role>` and mounts the generated API key:
  ...
  parameters:
    ...
    objects: |
      - objectName: "weaviate-api-key"
        secretPath: "your-database-path/creds/k8s.-.demo.weaviate-app-role"
        secretKey: "password"
```
...
Now we can create a `Pod` to consume the dynamic `Weaviate` API key. When the `Pod` is created, the `Provider` fetches the secret and writes it to Pod's volume as a file. At this point, the volume is successfully mounted and the `Pod` starts running.
...
/ # cat /secrets-store/weaviate-creds/weaviate-api-key
w-some-dynamic-api-key

/ # exit
```

The dynamic API key is mounted into the pod. The application can read this file and present it to Weaviate in the `Authorization: Bearer <token>` header.
