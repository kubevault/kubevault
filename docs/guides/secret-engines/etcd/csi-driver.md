---
title: Mount etcd Secrets using CSI Driver
menu:
  docs_{{ .version }}:
    identifier: csi-driver-etcd
    name: CSI Driver
...
# Mount etcd Secrets using CSI Driver
...
> Note: YAML files used in this tutorial stored in [examples](/docs/examples/guides/secret-engines/etcd) folder in GitHub repository [KubeVault/docs](https://github.com/kubevault/kubevault)
...
## Enable & Configure etcd SecretEngine

### Enable etcd SecretEngine

```bash
$ kubectl apply -f docs/examples/guides/secret-engines/etcd/secretengine.yaml
...
```bash
$ kubectl apply -f docs/examples/guides/secret-engines/etcd/secretenginerole.yaml
etcdrole.engine.kubevault.com/etcd-reader-role created
```
...
```bash
$ kubectl apply -f docs/examples/guides/secret-engines/etcd/serviceaccount.yaml
...
When a VaultPolicyBinding object is created, the KubeVault operator create an auth role in the Vault server. The role name is generated by the following naming format: `k8s.(clusterName or -).namespace.name`. Here, it is `k8s.kubevault.com.demo.etcd-reader-role`.
...
```bash
$ kubectl apply -f docs/examples/guides/secret-engines/etcd/secretrolebinding.yaml
...
spec:
  ...
  parameters:
    ...
    roleName: "k8s.-.demo.etcd-reader-role"
    objects: |
      - objectName: "etcd-creds-username"
        secretPath: "your-database-path/creds/k8s.-.demo.etcd-reader-role"
        secretKey: "username"
      - objectName: "etcd-creds-password"
        secretPath: "your-database-path/creds/k8s.-.demo.etcd-reader-role"
        secretKey: "password"
...
```bash
$ kubectl apply -f docs/examples/guides/secret-engines/etcd/secretproviderclass.yaml
...
Now we can create a `Pod` to consume the `etcd` secrets. When the `Pod` is created, the `Provider` fetches the secret and writes them to Pod's volume as files. At this point, the volume is successfully mounted and the `Pod` starts running.
    ...
    - image: jweissig/app:0.0.1
      ...
      volumeMounts:
        - name: secrets-store-inline
          mountPath: "/secrets-store/etcd-creds"
          readOnly: true
  volumes:
    - name: secrets-store-inline
      csi:
...
```bash
$ kubectl apply -f docs/examples/guides/secret-engines/etcd/pod.yaml
...
/ # ls /secrets-store/etcd-creds
etcd-creds-password  etcd-creds-username

/ # cat /secrets-store/etcd-creds/etcd-creds-password
TAu2Zvg1WYE07W8Uf-nW

/ # cat /secrets-store/etcd-creds/etcd-creds-username
v-kubernetes-test-k8s.-.demo.etcd-r-iPkxiH80Ollq2QgF82Ab-1629178048
