---
title: Mount Apache Kafka Secrets using CSI Driver
menu:
  docs_{{ .version }}:
    identifier: csi-driver-kafka
    name: CSI Driver
...
# Mount Apache Kafka Secrets using CSI Driver
...
> Note: YAML files used in this tutorial stored in [examples](/docs/examples/guides/secret-engines/kafka) folder in GitHub repository [KubeVault/docs](https://github.com/kubevault/kubevault)
...
## Enable & Configure Apache Kafka SecretEngine

### Enable Apache Kafka SecretEngine

```bash
$ kubectl apply -f docs/examples/guides/secret-engines/kafka/secretengine.yaml
...
```bash
$ kubectl apply -f docs/examples/guides/secret-engines/kafka/secretenginerole.yaml
kafkarole.engine.kubevault.com/kafka-superuser-role created
```
...
```bash
$ kubectl apply -f docs/examples/guides/secret-engines/kafka/serviceaccount.yaml
...
When a VaultPolicyBinding object is created, the KubeVault operator create an auth role in the Vault server. The role name is generated by the following naming format: `k8s.(clusterName or -).namespace.name`. Here, it is `k8s.kubevault.com.demo.kafka-superuser-role`.
...
```bash
$ kubectl apply -f docs/examples/guides/secret-engines/kafka/secretrolebinding.yaml
...
spec:
  ...
  parameters:
    ...
    roleName: "k8s.-.demo.kafka-reader-role"
    objects: |
      - objectName: "kafka-creds-username"
        secretPath: "your-database-path/creds/k8s.-.demo.kafka-superuser-role"
        secretKey: "username"
      - objectName: "kafka-creds-password"
        secretPath: "your-database-path/creds/k8s.-.demo.kafka-superuser-role"
        secretKey: "password"
...
```bash
$ kubectl apply -f docs/examples/guides/secret-engines/kafka/secretproviderclass.yaml
...
Now we can create a `Pod` to consume the `Kafka` secrets. When the `Pod` is created, the `Provider` fetches the secret and writes them to Pod's volume as files. At this point, the volume is successfully mounted and the `Pod` starts running.
    ...
    - image: jweissig/app:0.0.1
      ...
      volumeMounts:
        - name: secrets-store-inline
          mountPath: "/secrets-store/kafka-creds"
          readOnly: true
  volumes:
    - name: secrets-store-inline
      csi:
...
```bash
$ kubectl apply -f docs/examples/guides/secret-engines/kafka/pod.yaml
...
/ # ls /secrets-store/kafka-creds
kafka-creds-password  kafka-creds-username

/ # cat /secrets-store/kafka-creds/kafka-creds-password
TAu2Zvg1WYE07W8Uf-nW

/ # cat /secrets-store/kafka-creds/kafka-creds-username
v-kubernetes-test-k8s.-.demo.kafka-s-iPkxiH80Ollq2QgF82Ab-1629178048
