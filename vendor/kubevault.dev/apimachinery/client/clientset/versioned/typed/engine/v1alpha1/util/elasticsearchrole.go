...
func CreateOrPatchElasticsearchRole(
	...
	if kerr.IsNotFound(err) {
		glog.V(3).Infof("Creating ElasticsearchRole %s/%s.", meta.Namespace, meta.Name)
...
func PatchElasticsearchRoleObject(
	...
	glog.V(3).Infof("Patching ElasticsearchRole %s/%s with %s.", cur.Namespace, cur.Name, string(patch))
...
func TryUpdateElasticsearchRole(
	...
	err = wait.PollImmediate(kutil.RetryInterval, kutil.RetryTimeout, func() (bool, error) {
		...
		glog.Errorf("Attempt %d failed to update ElasticsearchRole %s/%s due to %v.", attempt, cur.Namespace, cur.Name, e2)
		return false, nil
	})
