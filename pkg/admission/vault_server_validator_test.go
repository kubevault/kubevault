...
import (
	...
	extfake "kubevault.dev/apimachinery/client/clientset/versioned/fake"
	clientsetscheme "kubevault.dev/apimachinery/client/clientset/versioned/scheme"
...
func TestVaultServerValidator_Admit(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			object:    func() api.VaultServer { v := validVaultServer(); v.Spec.Replicas = pointer.Int32P(10); return v }(),
...
func TestValidateVaultServer(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			vs:          func() *api.VaultServer { v := vs; v.Spec.Replicas = pointer.Int32P(0); return &v }(),
			extraSecret: nil,
			expectErr:   true,
		},
		{
