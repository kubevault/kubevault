...
func (vc *VaultController) runSecretWatcher() {
	for vc.processNextSecret() {
	}
}

func (vc *VaultController) processNextSecret() bool {
	// Wait until there is a new item in the working queue
	key, quit := vc.sQueue.Get()
	if quit {
		return false
	}
	...
	// parallel.
	defer vc.sQueue.Done(key)

	// Invoke the method containing the business logic
	err := vc.syncSToStdout(key.(string))
	if err == nil {
		// Forget about the #AddRateLimited history of the key on every successful synchronization.
		// This ensures that future processing of updates for this key is not delayed because of
		// an outdated error history.
		vc.sQueue.Forget(key)
		return true
	}

	// This controller retries 5 times if something goes wrong. After that, it stops trying.
	if vc.sQueue.NumRequeues(key) < vc.options.MaxNumRequeues {
		glog.Infof("Error syncing secret %v: %v", key, err)

		// Re-enqueue the key rate limited. Based on the rate limiter on the
		// queue and the re-enqueue history, the key will be processed later again.
		vc.sQueue.AddRateLimited(key)
		return true
	}
...
func (vc *VaultController) syncSToStdout(key string) error {
	obj, exists, err := vc.sIndexer.GetByKey(key)
	if err != nil {
