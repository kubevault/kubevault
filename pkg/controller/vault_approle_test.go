...
func TestReconcileAppRole(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.testName, func(t *testing.T) {
			...
			if c.expectStatus != "" {
				p, err := ctrl.extClient.ApproleV1alpha1().VaultAppRoles(c.vAppRole.Namespace).Get(c.vAppRole.Name, metav1.GetOptions{})
				if assert.Nil(t, err) {
...
func TestFinalizeAppRole(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.testName, func(t *testing.T) {
			...
			if c.vAppRole != nil {
				_, err := pc.VaultAppRoles(c.vAppRole.Namespace).Create(c.vAppRole)
				assert.Nil(t, err)
			} else {
