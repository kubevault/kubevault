...
func (c *VaultController) runVaultAppRoleInjector(key string) error {
	...
	if !exists {
		...
		if vAppRole.DeletionTimestamp != nil {
			...
			if !core_util.HasFinalizer(vAppRole.ObjectMeta, VaultAppRoleFinalizer) {
				// Add finalizer
				_, _, err := patchutil.PatchVaultAppRole(context.TODO(), c.extClient.ApproleV1alpha1(), vAppRole, func(vp *approleapi.VaultAppRole) *approleapi.VaultAppRole {
...
func (c *VaultController) reconcileAppRole(vAppRole *approleapi.VaultAppRole, aClient approle.AppRole) error {
	...
	if err2 != nil {
		...
		err3 := c.updateAppRoleStatus(&status, vAppRole, metav1.UpdateOptions{})
		if err3 != nil {
	...
	err4 := c.updateAppRoleStatus(&status, vAppRole, metav1.UpdateOptions{})
	if err4 != nil {
...
func (c *VaultController) updateAppRoleStatus(status *approleapi.VaultAppRoleStatus, vAppRole *approleapi.VaultAppRole, opts metav1.UpdateOptions) error {
	_, err := patchutil.UpdateVaultAppRoleStatus(context.TODO(), c.extClient.ApproleV1alpha1(), vAppRole.ObjectMeta, func(s *approleapi.VaultAppRoleStatus) *approleapi.VaultAppRoleStatus {
		return status
	}, opts)
	return err
}
...
func (c *VaultController) runAppRoleFinalizer(vAppRole *approleapi.VaultAppRole, timeout time.Duration, interval time.Duration) {
	...
	// Remove finalizer
	_, err := patchutil.TryPatchVaultAppRole(context.TODO(), c.extClient.ApproleV1alpha1(), vAppRole, func(in *approleapi.VaultAppRole) *approleapi.VaultAppRole {
...
func (c *VaultController) finalizeAppRole(vAppRole *approleapi.VaultAppRole) error {
	out, err := c.extClient.ApproleV1alpha1().VaultAppRoles(vAppRole.Namespace).Get(context.TODO(), vAppRole.Name, metav1.GetOptions{})
