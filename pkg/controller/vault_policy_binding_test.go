...
import (
	...
	csfake "kubevault.dev/apimachinery/client/clientset/versioned/fake"
...
func TestRunPolicyBindingFinalizer(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.testName, func(t *testing.T) {
			if err := ctrl.runPolicyBindingFinalizer(c.vPBind); (err != nil) != c.wantErr {
				t.Errorf("runPolicyBindingFinalizer() error = %v, wantErr %v", err, c.wantErr)
			}
		})
	}
}
